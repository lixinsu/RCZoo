...
from reader.bert.file_utils import PYTORCH_PRETRAINED_BERT_CACHE, WEIGHTS_NAME, CONFIG_NAME
from reader.bert.modeling import BertForQuestionAnswering, BertConfig
from reader.bert.optimization import BertAdam, WarmupLinearSchedule
from reader.bert.tokenization import (BasicTokenizer,
                                                  BertTokenizer,
                                                  whitespace_tokenize)
...
def main():
    ...
    parser.add_argument("--debug", action="store_true", help="Whether run on small dataset")
    ...
    pprint(args)
    updated_args = yaml.load(open(os.path.join(args.output_dir, 'params.yaml')))
    print('Updated Params')
    for k, v in updated_args.items():
        if not hasattr(args, k):
            raise KeyError('{} is not in args'.format(k))
        setattr(args, k, v)
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                   all_start_positions, all_end_positions, all_start_vector,
