...
from reader.multi_fusionnet import utils, vector, config, data
from reader.multi_fusionnet import DocReader
from reader import DATA_DIR as DRQA_DATA
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    # Build a pos dict from question  + contexts
    logger.info('-' * 100)
    logger.info('Build part-of-speech dictionary')
    pos_dict = utils.build_pos_dict(args, train_exs + dev_exs)
    logger.info('Num pos = %d' % len(pos_dict))

    # Build a ner dict from question + contexts
    logger.info('-' * 100)
    logger.info('Build name entity recognition dictionary')
    ner_dict = utils.build_ner_dict(args, train_exs + dev_exs)
    logger.info('Num pos = %s' % len(ner_dict))

    all_dicts = {'feature_dict': feature_dict, 'word_dict': word_dict, 'char_dict': char_dict, 'pos_dict': pos_dict, 'ner_dict': ner_dict}

    # Initialize model
    model = DocReader(config.get_model_args(args), all_dicts=all_dicts, normalize=True)
...
def decode(text,offsets,s,e):
    """
    Decode for extract answer from text based on the scores
    """
    start_indices = [ 1 if x > 0.5 else 0 for x in s ]
    end_indices = [ 1 if x > 0.5 else 0 for x in e ]
    answers = []
    for idx in range(len(start_indices)):
        if start_indices[idx] == 1:
            for end_idx in range(idx,len(end_indices)):
                if end_indices[end_idx] == 1:
                    answers.append(text[offsets[idx][0]:offsets[end_idx][1]])
                    break
    return answers
...
def validate_official(args, data_loader, model, global_stats,
    ...
    for ex in data_loader:
        ...
        score_s, score_e= model.predict(ex)

        for i in range(batch_size):

            pred_answers = decode(texts[ex_id[i]], offsets[ex_id[i]], score_s[i], score_e[i])

            # Compute metrics
            ground_truths = answers[ex_id[i]]
            TP = set(ground_truths) & set(pred_answers)
            if set(ground_truths) == set(pred_answers):
            ...
            if len(pred_answers) == 0:
                ...
                p = len(TP) * 1.0 / len(pred_answers)
                r = len(TP) * 1.0 / len(ground_truths)
                if p == 0 and  r == 0:
                    F1 = 0
                else:
                    F1 = 2 * p * r / (p+r)
