...
class RnnDocReader(nn.Module):
    ...
    def __init__(self, args, normalize=True):
        ...
        if args.use_qemb:
            self.qemb_match = layers.MultiheadAttention(args.embedding_dim, 4)
    ...
    def forward(self, x1, x1_f, x1_mask, x2, x2_mask):
        ...
        if self.args.use_qemb:
            x2_weighted_emb, _ = self.qemb_match(x1_emb.transpose(0,1), x2_emb.transpose(0,1), x2_emb.transpose(0,1), key_padding_mask=x2_mask, need_weights=False)
            drnn_input.append(x2_weighted_emb.transpose(0,1))
