...
def batchify(batch):
    ...
    if df[0] is None:
        ...
        x1_f = torch.zeros(len(docs), max_length, df[0].size(1))

    x1_mask = torch.ByteTensor(len(docs), max_length).fill_(1)

    for i, d in enumerate(docs):
        x1[i, :d.size(0)].copy_(docs[i])
        x1_p[i, :d.size(0)].copy_(dp[i])
        x1_e[i, :d.size(0)].copy_(de[i])
        x1_c[i, :d.size(0)].copy_(dc[i])
    ...
    for i, q in enumerate(questions):
        x2[i, :q.size(0)].copy_(questions[i])
        x2_p[i, :q.size(0)].copy_(qp[i])
        x2_e[i, :q.size(0)].copy_(qe[i])
